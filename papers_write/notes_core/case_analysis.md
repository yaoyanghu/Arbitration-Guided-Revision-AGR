...
Across the disjoint validation set, the dominant pattern for the `routeA_bm25_title_overlap` variant remains rank correction inside an already retrieved candidate set rather than broader candidate coverage. The improved cases still cluster around three interpretable behaviors:
...
This is consistent with the intended contribution of this variant: a lightweight title-aware reranking gain for evidence retrieval, not a change in candidate-set coverage and not a full end-to-end system improvement.
...
The effect therefore becomes slightly more conservative under the disjoint validation split, but the qualitative conclusion is unchanged: the title-overlap variant still improves strict rank-1 evidence retrieval in a stable and interpretable way relative to BM25, even though the broader paper claim should now be framed at the level of title-aware lexical reranking rather than title overlap alone.
